...
def add_vehicle_txn(session, longitude, latitude, battery, vehicle_info):
    ...
    new_vehicle_row = Vehicle(id=str(vehicle_id),
                              in_use=False,
                              vehicle_info=vehicle_info,  # JSON lab solution
                              battery=battery)
...
def get_vehicles_txn(session, max_records):
    ...
    * Single-table query was session.query(Vehicle).limit(max_records).all()
    ...
    SELECT
        v.id AS id,
        v.in_use AS in_use,
        v.vehicle_info AS vehicle_info,  # Added for JSON column
    ...
    * Single-table query was session.query(Vehicle).limit(max_records).all()
    ...
    vehicles = session.query(v.id, v.in_use,
                             v.vehicle_info,  # Added for JSON column
                             v.battery,
...
def get_vehicle_txn(session, vehicle_id):
    ...
    return {'id': str(vehicle.id), 'last_longitude': vehicle.longitude,
            ...
            'vehicle_info': vehicle.vehicle_info  # Added for JSON column
            }
...
def get_rides_by_user_txn(session, email):
    ...
    return list(map(lambda ride: {'vehicle_id': ride.vehicle_id,
                                  # Added the following for JSON column
